...
1. Соединяем второе и первое задание, то есть измененные гугл таблицы с функцией loss и Unity, то есть изменяем диапазон данных и количество колонок
![image](https://user-images.githubusercontent.com/87576995/194136355-3b76d9d8-7167-42b8-8a14-de7a1a950e36.png)
...
public class NewBehaviourScript : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (dataSet["Mon_" + i.ToString()] > 100 & dataSet["Mon_" + i.ToString()] < 400 & statusStart == false & i != dataSet.Count)
        {
            StartCoroutine(PlaySelectAudioNormal());
            Debug.Log(dataSet["Mon_" + i.ToString()]);
        }
